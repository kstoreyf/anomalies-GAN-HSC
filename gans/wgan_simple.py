# from https://github.com/wiseodd/generative-models/blob/master/GAN/improved_wasserstein_gan/wgan_gp_tensorflow.py
import matplotlib
matplotlib.use('Agg')
...
mb_size = 32
X_dim = 784 # image dim (28 x 28)
z_dim = 10
h_dim = 128 # hidden dim?
lam = 10
n_disc = 5
lr = 1e-4
...
for it in range(iters):
    ...
    if it % logat == 0 or it==iters-1:
        print('Iter: {}; D loss: {:.4}; G_loss: {:.4}'
              .format(it, D_loss_curr, G_loss_curr))
        np.savetxt(f,np.array([iter_arr, D_loss_arr, G_loss_arr]).T,fmt='%i %f %f')
        #clean batch
        iter_arr = []
        G_loss_arr = []
        D_loss_arr = [] 
